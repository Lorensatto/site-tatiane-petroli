...
if(form){
 ...
 fetch(dataUrl).then(r=>{if(!r.ok)throw Error();return r.json()}).then(data=>{
  ...
  function render(){let items=data.filter(p=>p.status==='disponivel'&&(!form.elements.finalidade.value||p.finalidade===form.elements.finalidade.value)&&(!form.elements.tipo.value||p.tipo===form.elements.tipo.value)&&(!city.value||p.cidade===city.value)&&(!hood.value||p.bairro===hood.value)&&(!form.elements.preco.value||p.preco<=Number(form.elements.preco.value))&&(!form.elements.ref.value||normalize(p.ref).includes(normalize(form.elements.ref.value))));
   ...
   results.innerHTML=items.map(p=>`<a class="card" href="imovel-${p.slug}.html"><div class="card-image"><img loading="lazy" src="${p.imagem}" alt="Foto do imóvel ${p.titulo}"><span class="pill">${p.finalidade}</span></div><div class="card-content"><p class="eyebrow">${[p.bairro||p.endereco,p.cidade].filter(Boolean).join(' · ')}</p><h3>${p.titulo}</h3><div class="card-specs">${p.area?`<span>${p.area} m²</span>`:''}${p.quartos?`<span>${p.quartos} dormitórios</span>`:''}${p.vagas?`<span>${p.vagas} vagas</span>`:''}</div><div class="card-bottom"><div><small>${p.finalidade==='Aluguel'?'Aluguel mensal':'Valor de venda'}</small><strong>${money(p.preco)}</strong></div><span class="round-arrow" aria-hidden="true">↗</span></div></div></a>`).join('');
